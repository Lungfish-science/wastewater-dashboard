...
ALL_ORF_TSV = Path.cwd() / "data" /  "VariantPMsLong.tsv"
LINEAGE_JSON = Path.cwd() / "data" / "major_lineages.json"
...
```{python}
render_for_quarto(orf_datasets, "ORF1a")
```
...
```{python}
render_for_quarto(orf_datasets, "ORF1b")
```
...
Metadata file available for all recently reviewed submissions available <a href="../data/found_meta.tsv.gz" download="found_meta.tsv.gz">here</a>.
...
```{python}
project_data = pl.read_csv("./data/found_meta.tsv.gz", separator="\t", has_header=True, null_values=["NA", ">", "<"], schema_overrides={"Population":pl.Utf8})
